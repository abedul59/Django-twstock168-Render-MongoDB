...
from module_PERseg import PERsegxOTCYearPrice
...
#PERsegx
#############################################################以下為本益比區間程式
def PERsegxMain(stock_id, month_id):
    ...
    #try:  #查詢上市年股價 （先上市，有些股票會上櫃轉上市，但上櫃還會留下資料，會抓錯。）
        ...
        if eps1N == '2023':  #在3/31前，Q4財報先出來的情況  西元年度轉換時還不適用
                        
    ####2020/07/02 加入本益比成長率    
        #2021/1/6更改
            PER_H1 = round(float(H1)/float(eps1),2)  #112/2023本益比 高
            PER_L1 = round(float(L1)/float(eps1),2)  #   本益比 低 

            PER_H2 = round(float(H2)/float(eps2),2)  #111本益比 高
            PER_L2 = round(float(L2)/float(eps2),2)  #   本益比 低 

            PER_H3 = round(float(H3)/float(eps3),2)  #110本益比 高
            PER_L3 = round(float(L3)/float(eps3),2)  #   本益比 低 
            
            PER_H4 = round(float(H4)/float(eps4),2)  #109本益比 高
            PER_L4 = round(float(L4)/float(eps4),2)  #   本益比 低 
    
            PER_H5 = round(float(H5)/float(eps5),2)  #108本益比 高
            PER_L5 = round(float(L5)/float(eps5),2)  #   本益比 低 
...
#print(PER_H1)
        ...
        elif eps1N == '2022':  #3/31前，Q4財報還沒出來的情況 沒有2023的全年EPS
    ####2020/07/02 加入本益比成長率
            #2021EPS還沒出來的情況   #2021/12/31修改 欄位已改變 12/30最後交易日
        #2021/1/6更改  H1為2020高點
            PER_H1 = round(float(H2)/float(eps1),2)  #112/2023本益比 高
            PER_L1 = round(float(L2)/float(eps1),2)  #112本益比 低 

            PER_H2 = round(float(H3)/float(eps2),2)  #111本益比 高
            PER_L2 = round(float(L3)/float(eps2),2)  #   本益比 低 

            PER_H3 = round(float(H4)/float(eps3),2)  #110本益比 高
            PER_L3 = round(float(L4)/float(eps3),2)  #   本益比 低 

            PER_H4 = round(float(H5)/float(eps4),2)  #109本益比 高
            PER_L4 = round(float(L5)/float(eps4),2)  #   本益比 低 
    
            PER_H5 = round(float(H6)/float(eps5),2)  #108本益比 高
            PER_L5 = round(float(L6)/float(eps5),2)  #   本益比 低 
...
################################################接著取得上市股票EPS
        #取得上櫃年股價 EPS 本益比
        H0, H1, L1, H2, L2, H3, L3, H4, L4, H5, L5, H6, L6, eps1, eps2, eps3, eps4, eps5, PER_H1, PER_L1, PER_H2, PER_L2, PER_H3, PER_L3, PER_H4, PER_L4, PER_H5, PER_L5, PER_L5, PER_H_average, PER_L_average, PER_H, PER_L, eps1N = PERsegxOTCYearPrice.getOtcYearPriceEPS(stock_id)

            


    #取得季EPS 某些時候用
    epsq1N, epsq1, epsq2, epsq3, epsq4 = PERsegxGetQuarterEPS.getQuarterEPS(stock_id)
    
##############################################取得營收######################################

    #取得營收
    r1N, r2N, r3N, r4N, r5N, r6N, r7N, r8N, r9N, r10N, r11N, r12N, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, Rev_Predict, Q4_Rev_Predict, rYoY1N, rYoY2N, rYoY3N, rYoY4N, rYoY5N, rYoY6N, rYoY1, rYoY2, rYoY3, rYoY4, rYoY5, rYoY6, pYoY1, pYoY2, pYoY3, pYoY4, pYoY5, pYoY6, pYoY6Average, rYoY6Average, pRevYoY, RevYoY, thisYear_Sum, theRest_Predict = PERsegxGetRev.getRev(stock_id, month_id, epsq1N)



##################################取得稅後淨利率
    Net1N, Net2N, Net3N, Net4N, Net1, Net2, Net3, Net4, pNet1, pNet2, pNet3, pNet4, Net4Average, pNet4Average, Net_Predict, Q4_Net_Predict = PERsegxGetNet.getNet(stock_id, eps1N, Rev_Predict, Q4_Rev_Predict)

####################################################取得股本
    cap1 = PERsegxGetCap.getCap(stock_id)
#######################################################
...
####################計算預估EPS
    ...
    if eps1N == '2023' :
        ...
        #pass
        Q4_Predict_EPS = round(Q4_Net_Predict/capital_stock*10,2) #估算2023/Q4 eps
        Predict_EPS0 = epsq1 + epsq2 + epsq3 + Q4_Predict_EPS #估2023全年EPS 2024/3/31前 尚未公佈2023Q4財報
#print(Predict_EPS)
    print(PER_H)
...
########################取得目前股價
    yahoo_latest_tradePrice = PERsegxGetPrice.getPrice(stock_id)

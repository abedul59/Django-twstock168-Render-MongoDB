...
def DCstock_Prof(stock_id):  #營業利益率評分標準
...
#print(a1)
    ...
    if (0 < a9 <= 5):
        resultB2 = 1      #或不論漲跌幅，過去四季平均營益率在5%以下
    else:
        resultB2 = -1

        
    if (a10 <= -0.2):
        resultB1 = 1      #最近一季比上一季跌20%以上
    else:
        resultB1 = -1


    if (a11 <= -0.2):
        resultBB1 = 2     #過去四季曾出現季與季之間下跌20%以上，但不包括最近一季 1
    else:
        resultBB1 = -1
        
        
    if (a12 <= -0.2):
        resultBB2 = 2     #過去四季曾出現季與季之間下跌20%以上，但不包括最近一季 2
    else:
        resultBB2 = -1


    if (15 >= a9 >= 10) and (a1 > a2) and (a2 > a3*0.8) and (a3 > a4*0.8):
        resultAA2 = 4     #或過去四季皆維持穩定沒有下降，且平均在10-15%，最近一季呈現上升趨勢
    else:
        resultAA2 = -1


    if (15 >= a9 >= 10) and (a1 > a2*0.8) and (a2 > a3*0.8) and (a3 > a4*0.8): 
        resultA1 = 3      #過去四季皆維持穩定沒有下降，且平均在10-15%
    else:
        resultA1 = -1

        
    if (10 > a9 >= 5) and (a1 > a2) and (a2 > a3*0.8) and (a3 > a4*0.8):
        resultA2 = 3      #或過去四季皆維持穩定沒有下跌，平均只有5-10%，但最近一季呈現上升趨勢
    else:
        resultA2 = -1


    if (a9 > 15) and (a1 > a2*0.8) and (a2 > a3*0.8) and (a3 > a4*0.8):
        resultAA1 = 4   #過去四季皆維持穩定沒有下跌，且平均在15%以上
    else:
        resultAA1 = -1

    if resultAA1 == resultAA2 == resultA1 == resultA2 == resultBB1 == resultBB2 == resultB1 == resultB2 == resultC1 == resultC2 == -1:
        resultBB3 = 2  #無法歸類的情況
    else:
        resultBB3 = -1
    ...
    return resultAA1, resultAA2, resultA1, resultA2, resultBB1, resultBB2, resultBB3, resultB1, resultB2, resultC1, resultC2, a1N, a2N, a3N, a4N, a5N, a6N, a7N, a8N, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a10p
    #result1, ProfitN, Profit, a1N, a2N, a3N, a4N, a5N, a6N, a7N, a8N, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a10p

################################################################

def DCstock_Rev(stock_id): #(股票代碼, 股票名稱. 股票價格觸及下緣) #營收評分標準
    headers = {'Referer':my_Referer,'user-agent': my_UserAgent}
...
#dfs[2][4][6] 最新一個月年增率
    ...
    b7 = round(float((dfs[2][4][12])[:-1])/100,4) #最新7個月的營收年增率 

    Rev = []
    Rev.extend([b1,b2,b3,b4,b5,b6,b7])
    ...
    b9 = (b1+b2+b3+b4+b5+b6)/6 #無農曆月份 6個月平均   小數
    nb9 = str(round(b9*100,2))  #無農曆月份 6個月平均 百分比
    ...
    if b1Nm == "02":   #近六個內有一二月的判斷式，2月為最新月份
        if ((lu2_1n2+b3+b4+b5+b6+b7)/6 < 0):
            resultC1 = 0      #過去6個月平均數為負
        else:
            resultC1 = -1
    
    
        if (lu2_1n2 < 0): #或最近一個月為負數  ，最新一個月是1,2月
            resultC2 = 0
        else:
            resultC2 = -1


        if (lu2_1n2 < 0 or b3 < 0 or b4 < 0 or b5 < 0 or b6 < 0 or b7 < 0):
            resultBB1 = 2         #過去六個月曾經出現單月負成長
        else:
            resultBB1 = -1


        if (lu2_1n2 < b3 < b4 and (lu2_1n2+b3+b4+b5+b6+b7)/6 > 0): #2,1,12,11月遞減的情況
            resultB1 = 1      #過去六個月平均為正數，但最近三個月出現遞減（不論幅度）
        else:
            resultB1 = -1


        if ((lu2_1n2+b3+b4+b5+b6+b7)/6 > 0.25 and 0 > (lu2_1n2-b3)/b3 >= -0.5):
            resultA1 = 3      #或過去六個月平均超過25%，但最近一個月出現小幅衰退。（年增率較上個月下跌幅度在50%以內） 
        else:
            resultA1 = -1

        if (lu2_1n2 > 0 and b3 > 0 and b4 > 0 and b5 >0 and b6 > 0 and b7 > 0 and 0.25 >= (lu2_1n2+b3+b4+b5+b6+b7)/6 >= 0.1 and lu2_1n2 >= b3):
            resultA2 = 3      #過去六個月皆為正數平均10-25%，且最近一個月的年增率與上個月比較為增加或持平（>=0）    
        else:
            resultA2 = -1
            
        
        if (lu2_1n2 > 0 and b3 > 0 and b4 > 0 and b5 >0 and b6 >0 and b7 > 0 and (lu2_1n2+b3+b4+b5+b6+b7)/6 > 0.25 and lu2_1n2 >= b3):
            resultAA1 = 4     #過去六個月皆為正數且平均超過25%，且最近一個月的年增率與上個月比為增加或持平（>=0）
        else:
            resultAA1 = -1
                        
        if resultAA1 == resultA1 == resultA2 == resultBB1 == resultB1 == resultC1 == resultC2 == -1:
            resultBB3 = 2  #無法歸類的情況
        else:
            resultBB3 = -1        
        #else:
...
###########################            
        ...
        if (b1 < 0): #或最近一個月為負數，最新一個月是3月
            resultC2 = 0
        else:
            resultC2 = -1

        if (b1 < 0 or lu3_1n2 < 0 or b4 < 0 or b5 < 0 or b6 < 0 or b7): #3,2,1,12,11,10,9月
            resultBB1 = 2         #過去六個月曾經出現單月負成長
        else:
            resultBB1 = -1

        if (b1 < lu3_1n2 < b4 and (b1+lu3_1n2+b4+b5+b6+b7)/6 > 0): #3,2,1月遞減的情況
            resultB1 = 1      #過去六個月平均為正數，但最近三個月出現遞減（不論幅度）
        else:
            resultB1 = -1

        if ((b1+lu3_1n2+b4+b5+b6+b7)/6 > 0.25 and 0 > (b1-lu3_1n2)/lu3_1n2 >= -0.5):
            resultA1 = 3      #或過去六個月平均超過25%，但最近一個月出現小幅衰退。（年增率較上個月下跌幅度在50%以內）
        else:
            resultA1 = -1

        if (b1 > 0 and lu3_1n2 > 0 and b4 > 0 and b5 >0 and b6 > 0 and b7 > 0 and 0.25 >= (b1+lu3_1n2+b4+b5+b6+b7)/6 >= 0.1 and b1 >= lu3_1n2):
            resultA2 = 3      #過去六個月皆為正數平均10-25%，且最近一個月的年增率與上個月比較為增加或持平（>=0） 
        else:
            resultA2 = -1


        if (b1 > 0 and lu3_1n2 > 0 and b4 > 0 and b5 >0 and b6 >0  and b7 > 0 and (b1+lu3_1n2+b4+b5+b6+b7)/6 > 0.25 and b1 >= lu3_1n2):
            resultAA1 = 4    #過去六個月皆為正數且平均超過25%，且最近一個月的年增率與上個月比為增加或持平（>=0）
        else:
            resultAA1 = -1
...
######################
        ...
        if (b1 < 0): #或最近一個月為負數，最新一個月是4月
            resultC2 = 0
        else:
            resultC2 = -1


        if (b1 < 0 or b2 < 0 or lu4_1n2 < 0 or b5 < 0 or b6 < 0 or b7 < 0): #4,3,2,1,12,11,10,9月
            resultBB1 = 2         #過去六個月曾經出現單月負成長
        else:
            rsultBB1 = -1
            
        if (b1 < b2 < lu4_1n2 and (b1+b2+lu4_1n2+b5+b6+b7)/6 > 0): #4,3,2,1月遞減的情況
            resultB1 = 1      #過去六個月平均為正數，但最近三個月出現遞減（不論幅度）
        else:
            resultB1 = -1

        if ((b1+b2+lu4_1n2+b5+b6+b7)/6 > 0.25 and 0 > b10 >= -0.5):
            resultA1 = 3      #或過去六個月平均超過25%，但最近一個月出現小幅衰退。（年增率較上個月下跌幅度在50%以內）  
        else:
            resultA1 = -1

        if (b1 > 0 and b2 > 0 and lu4_1n2 > 0 and b5 >0 and b6 > 0 and b7 > 0 and 0.25 >= (b1+b2+lu4_1n2+b5+b6+b7)/6 >= 0.1 and b1 >= b2):
            resultA2 = 3      #過去六個月皆為正數平均10-25%，且最近一個月的年增率與上個月比較為增加或持平（>=0） 
        else:
            resultA2 = -1


        if (b1 > 0 and b2 > 0 and lu4_1n2 > 0  and b5 >0 and b6 >0 and b7 > 0 and (b1+b2+lu4_1n2+b5+b6+b7)/6 > 0.25 and b1 >= b2):
            resultAA1 = 4     #過去六個月皆為正數且平均超過25%，且最近一個月的年增率與上個月比為增加或持平（>=0）
        else:
            resultAA1 = -1
...
################
        ...
        if (b1 < 0): #或最近一個月為負數，最新一個月是5月
            resultC2 = 0
        else:
            resultC2 = -1

        if (b1 < 0 or b2 < 0 or b3 < 0 or lu5_1n2 < 0 or b6 < 0 or b7 < 0): #5,4,3,2,1,12,11月
            resultBB1 = 2         #過去六個月曾經出現單月負成長
        else:
            resultBB1 = -1

        if (b1 < b2 < b3 and (b1+b2+b3+lu5_1n2+b6+b7)/6 > 0): #5,4,3月遞減的情況
            resultB1 = 1      #過去六個月平均為正數，但最近三個月出現遞減（不論幅度）
        else:
            resultB1 = -1

        if ((b1+b2+b3+lu5_1n2+b6+b7)/6 > 0.25 and 0 > b10 >= -0.5):
            resultA1 = 3      #或過去六個月平均超過25%，但最近一個月出現小幅衰退。（年增率較上個月下跌幅度在50%以內）
        else:
            resultA1 = -1

        if (b1 > 0 and b2 > 0 and b3 > 0  and lu5_1n2 >0 and b6 > 0 and b7 > 0 and 0.25 >= (b1+b2+b3+lu5_1n2+b6+b7)/6 >= 0.1 and b1 >= b2):
            resultA2 = 3      #過去六個月皆為正數平均10-25%，且最近一個月的年增率與上個月比較為增加或持平（>=0）
        else:
            resultA2 = -1        


        if (b1 > 0 and b2 > 0 and b3 > 0  and lu5_1n2 >0 and b6 >0 and b7 > 0 and (b1+b2+b3+lu5_1n2+b6+b7)/6 > 0.25 and b1 >= b2):
            resultAA1 = 4     #過去六個月皆為正數且平均超過25%，且最近一個月的年增率與上個月比為增加或持平（>=0）
        else:
            resultAA1 = -1
...
##########################
        ...
        if (b1 < 0): #或最近一個月為負數，最新一個月是6月
            resultC2 = 0
        else:
            resultC2 = -1


        if (b1 < 0 or b2 < 0 or b3 < 0 or b4 < 0 or lu6_1n2 < 0 or b7 < 0): #6,5,4,3,2,1,12月
            resultBB1 = 2         #過去六個月曾經出現單月負成長
        else:
            resultBB2 = -1

        if (b1 < b2 < b3 and (b1+b2+b3+b4+lu6_1n2+b7)/6 > 0): #6,5,4月遞減的情況
            resultB1 = 1      #過去六個月平均為正數，但最近三個月出現遞減（不論幅度）
        else:
            resultB1 = -1

        if ((b1+b2+b3+b4+lu6_1n2+b7)/6 > 0.25 and 0 > b10 >= -0.5):
            resultA1 = 3      #或過去六個月平均超過25%，但最近一個月出現小幅衰退。（年增率較上個月下跌幅度在50%以內）
        else:
            resultA1 = -1

        if (b1 > 0 and b2 > 0 and b3 > 0  and b4 >0 and lu6_1n2 >0 and b7 > 0 and 0.25 >= (b1+b2+b3+b4+lu6_1n2+b7)/6 >= 0.1 and b1 >= b2):
            resultA2 = 3      #過去六個月皆為正數平均10-25%，且最近一個月的年增率與上個月比較為增加或持平（>=0）
        else:
            resultA2 = -1


        if (b1 > 0 and b2 > 0 and b3 > 0  and b4 >0 and lu6_1n2 >0 and b7 > 0 and (b1+b2+b3+b4+lu6_1n2+b7)/6 > 0.25 and b1 >= b2):
            resultAA1 = 4     #過去六個月皆為正數且平均超過25%，且最近一個月的年增率與上個月比為增加或持平（>=0）
        else:
            resultAA1 = -1
        ...
        if (b1 < 0): #或最近一個月為負數，最新一個月是7月
            resultC2 = 0
        else:
            resultC2 = -1

        if (b1 < 0 or b2 < 0 or b3 < 0 or b4 < 0 or b5 < 0 or lu7_1n2 < 0): #7,6,5,4,3,2,1月
            resultBB1 = 2         #過去六個月曾經出現單月負成長
        else:
            resultBB1 = -1

        if (b1 < b2 < b3 and (b1+b2+b3+b4+b5+lu7_1n2)/6 > 0): #7,6,5月遞減的情況
            resultB1 = 1      #過去六個月平均為正數，但最近三個月出現遞減（不論幅度）
        else:
            resultB1 = -1

        if ((b1+b2+b3+b4+b5+lu7_1n2)/6 > 0.25 and 0 > b10 >= -0.5):
            resultA1 = 3      #或過去六個月平均超過25%，但最近一個月出現小幅衰退。（年增率較上個月下跌幅度在50%以內）
        else:
            resultA1 = -1

        if (b1 > 0 and b2 > 0 and b3 > 0  and b4 >0 and b5 > 0 and lu7_1n2 > 0 and 0.25 >= (b1+b2+b3+b4+b5+lu7_1n2)/6 >= 0.1 and b1 >= b2):
            resultA2 = 3      #過去六個月皆為正數平均10-25%，且最近一個月的年增率與上個月比較為增加或持平（>=0）
        else:
            resultA2 = -1

        if (b1 > 0 and b2 > 0 and b3 > 0  and b4 >0 and b5 > 0 and lu7_1n2 > 0 and (b1+b2+b3+b4+b5+lu7_1n2)/6 > 0.25 and b1 >= b2):
            resultAA1 = 4     #過去六個月皆為正數且平均超過25%，且最近一個月的年增率與上個月比為增加或持平（>=0）
        else:
            resultAA1 = -1
        ...
        if (b1 < 0):
            resultC2 = 0      #或最近一個月為負數
        else:
            resultC2 = -1

        if (b1 < 0 or b2 < 0 or b3 < 0 or b4 < 0 or b5 < 0 or b6 < 0):
            resultBB1 = 2         #過去六個月曾經出現單月負成長
        else:
            resultBB1 = -1

        if (b1 < b2 < b3 and b9 > 0):
            resultB1 = 2      #過去六個月平均為正數，但最近三個月出現遞減（不論幅度）
        else:
            resultB1 = -1

        if (b9 > 0.25 and 0 > b10 >= -0.5):
            resultA1 = 3      #或過去六個月平均超過25%，但最近一個月出現小幅衰退。（年增率較上個月下跌幅度在50%以內） 
        else:
            resultA1 = -1

        if (b1 > 0 and b2 > 0 and b3 > 0 and b4 > 0 and b5 >0 and b6 > 0 and 0.25 >= b9 >= 0.1 and b1 >= b2):
            resultA2 = 3      #過去六個月皆為正數平均10-25%，且最近一個月的年增率與上個月比較為增加或持平（>=0）
        else:
            resultA2 = -1
       
        if (b1 > 0 and b2 > 0 and b3 > 0 and b4 > 0 and b5 >0 and b6 >0 and b9 > 0.25 and b1 >= b2):
            resultAA1 = 4     #過去六個月皆為正數且平均超過25%，且最近一個月的年增率與上個月比為增加或持平（>=0）
        else:
            resultAA1 = -1
    ...
    #print(b1_2)
    #print(result2)
    return resultAA1, resultA1, resultA2, resultBB1, resultBB3, resultB1, resultC1, resultC2, b1N, b2N, b3N, b4N, b5N, b6N, nb1, nb2, nb3, nb4, nb5, nb6, nb9, nb10, b7N, nb7
#result2, RevN, Rev, b1N, b2N, b3N, b4N, b5N, b6N, nb1, nb2, nb3, nb4, nb5, nb6, nb9, nb10, b7N, nb7
...
def DCstock_NetInc(stock_id): #税後淨利年增率
    headers = {'Referer':my_Referer,'user-agent': my_UserAgent}
...
#print(c13)
    ...
    if (c9 < c10 < c11 and c9 < 0.5):
        resultB1 = 1     #或近三季遞減（且最近一季税後淨利年增率低於50%）
    else:
        resultB1 = -1
    ...
    if (c13 >= 2):
        resultB3 = 1     #或過去四季出現兩季負數
    else:
        resultB3 = -1
        

    if (c9 > 0 and c10 > 0 and c11 > 0 and c9 > c10): 
        resultAA1 = 4    #近三季皆為正數且最近一季呈現成長
    else:
        resultAA1 = -1
        
    if (c9 >= 0.5 and c10 >= 0.5 and c11 >= 0.5):
        resultAA2 = 4  #或近三季皆在50%以上（遞減也無妨）
    else:
        resultAA2 = -1


    if (c9 > 0 and c10 > 0 and (c9-c10)/c10 > -0.5):
        resultA1 = 3     #近兩季皆為正數且沒有出現大幅衰退
    else:
        resultA1 = -1

    if (c10 < 0 and c9 > 0):
        resultBB1 = 2    #或最近一季由負轉正
    else:
        resultBB1 = -1
        
    if (c9 > 0 and c10 > 0 and (c9-c10)/c10 <= -0.5):
        resultBB2 = 2    #近兩季皆為正數但最近一季出現50%以上的衰退
    else:
        resultBB2 = -1

    if resultAA1 == resultA1 == resultBB1 == resultBB2 == resultB1 == resultB2 == resultB3 == resultC1 == -1:
        resultBB3 = 2  #無法歸類的情況
    else:
        resultBB3 = -1 

    return resultC1, resultB1, resultB2, resultB3, resultBB1, resultBB2, resultBB3, resultA1, resultAA1, resultAA2, c1N, c2N, c3N, c4N, c5N, c6N, c7N, c8N, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, pc9, pc10, pc11
...
#print(d13)
    ...
    if (d1 < 0):
        resultB1 = 1   #或不管最近四季累積數，最近一季出現虧損者
    else:
        resultB1 = -1
        
    if (1 > d13 > 0):
        resultB2 = 1   #最近四季累積超過0元
    else:
        resultB2 = -1
        
    if (3 >= d13 >= 1):
        resultBB1 = 2  #最近四季累積1-3元
    else:
        resultBB1 = -1

    if (5 >= d13 > 3):
        resultA1 = 3   #最近四季累積3-5元
    else:
        resultA1 = -1
    ...
    #print (result4)
    return resultC1, resultB1, resultB2, resultBB1, resultA1, resultAA1, d1N, d2N, d3N, d4N, d5N, d6N, d7N, d8N, d1, d2, d3, d4, d5, d6, d7, d8
#, EPSN, EPS

##############################################################

def DCstock_InvTO(stock_id): #(股票代碼)
    headers = {'Referer':my_Referer,'user-agent': my_UserAgent}
...
#stock_id_name = dfs[2][1] #由左至右，第一欄數據
    ...
    if (e10 <= -0.2):
        resultC1 = 0    #最近一季出現20%以上的跌幅
    else:
        resultC1 = -1
        
    if (e11 <= -0.2 or e12 <= -0.2):
        resultB1 = 1    #最近四季曾經出現單季20%以上的跌幅
    else:
        resultB1 = -1
        

    if (e10 < 0 and e11 < 0 and (e1-e3)/e3 <= -0.2): 
        resultBB1 = 2   #最近四季出現連續兩季下跌，累積跌幅在20%以上1
    else:
        resultBB1 = -1
        
    if (e11 < 0 and e12 < 0 and (e2-e4)/e4 <= -0.2): 
        resultBB2 = 2   #最近四季出現連續兩季下跌，累積跌幅在20%以上2
    else:
        resultBB2 = -1

    if (e9 < 1.5 and e10 > -0.2 and e11 > -0.2 and e12 > -0.2):
        resultA1 = 3    #最近四季穩定不下降，且平均在1.5次以下
    else:
        resultA1 = -1

    if (e9 >= 1.5 and e10 > -0.2 and e11 > -0.2 and e12 > -0.2):
        resultAA1 = 4   #最近四季穩定不下降，且平均在1.5次以上
    else:
        resultAA1 = -1



        #if resultAA1 == resultA1 == resultBB1 == resultBB2 == resultB1  == resultC1 == -1:
            #resultNo2 = "不評分"  #無法歸類的情況
        #else:
            #resultNo2 = -1         
    ...
    return resultNo1, resultC1, resultB1, resultBB1, resultBB2, resultA1, resultAA1, e1N, e2N, e3N, e4N, e5N, e6N, e7N, e8N, e1, e2, e3, e4, e5, e6, e7, e8
##########################################################


def DCstock_Cashflow(stock_id): #(股票代碼, 股票名稱. 股票價格觸及下緣)
    headers = {'Referer':my_Referer,'user-agent': my_UserAgent}
...
#print(d11)
    ...
    if (f9 < 0 and f10 < 0):
        resultC1 = 0     #最近六季累積為負數且最近四季累積為負數
    else:
        resultC1 = -1
        
    if (f9 < 0 and f10 > 0):
        resultB1 = 1     #最近六季累積為正數且最近四季累積為負數
    else:
        resultB1 = -1
        
    if (f9 > 0 and f10 < 0):
        resultBB1 = 2     #最近六季累積為負數且最近四季累積為正數
    else:
        resultBB1 = -1

    if (f1 > 0 and f2 > 0 and f3 > 0 and f4 > 0 and f5 > 0 and f6 > 0):
        resultAA1 = 4     #連續六季出現正數
    else:
        resultAA1 = -1

    if (f10 > 0 and f9 > 0):
        resultA1 = 3     #最近六季累積為正數且最近四季累積為正數
    else:
        resultA1 = -1


    return resultC1, resultB1, resultBB1, resultA1, resultAA1, f1N, f2N, f3N, f4N, f5N, f6N, f7N, f8N, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10
...
def stock6score(stock_id, st1, st2, st3, st4, st5, st6):  #由stock6改為stock6score 2020/5/7##計算六大指標平均 
    ...
    st2, dt2N, dt2, b1N, b2N, b3N, b4N, b5N, b6N, b1, b2, b3, b4, b5, b6, nb9, nb10, b7N, nb7 = stock_Rev(stock_id)  #營收指標函數
    #print('營收指標為：' + st2)
...
##############################################################
#https://accounts.pixiv.net/loginlang=zh&source=pc&view_type=page&ref=wwwtop_accounts_index
...
############Mdj營收最新月份
    ...
    bank_url = 'https://dj.mybank.com.tw/' #國泰世華 #不能隨便改 會發生錯誤 2020/10/22
    sheet_type = 'z/zc/zch/zch_' #Rev 營收   
    url = bank_url + sheet_type + stock_id + '.djhtm'
...
def stockdef(stock_id): #(股票代碼, 股票名稱. 股票價格觸及下緣)
    ...
    bank_url = 'https://dj.mybank.com.tw/' #國泰世華
    sheet_type = 'z/zc/zca/zca_' #基本資料
    ###MoneyDJ
    url = bank_url + sheet_type + stock_id +'.djhtm'
    r = requests.get(url, headers=headers)

...
########################2021/2/28新增單季EPS計算 取代整年營收估EPS計算
    ...
    #eps1N = dfs5[2][1][1] #最新1季的名稱
    #eps2N = dfs5[2][2][1] #最新2季的名稱
    #eps3N = dfs5[2][3][1] #最新3季的名稱
    #eps4N = dfs5[2][4][1] #最新4季的名稱
    #b5N = dfs5[2][5][1] #最新5季的名稱
    #b6N = dfs5[2][6][1] #最新6季的名稱
    #b7N = dfs5[2][7][1] #最新7季的名稱
    #b8N = dfs5[2][8][1] #最新8季的名稱
    

    eps1 = float((dfs5[2][1][25])) #最新1季的EPS
    eps2 = float((dfs5[2][2][25])) #最新2季的EPS
    eps3 = float((dfs5[2][3][25])) #最新3季的EPS
    eps4 = float((dfs5[2][4][25])) #最新4季的EPS
    #b5 = float((dfs5[2][5][25])) #最新5季的EPS
    #b6 = float((dfs5[2][6][25])) #最新6季的EPS
    #b7 = float((dfs5[2][7][25])) #最新7季的EPS
    #b8 = float((dfs5[2][8][25])) #最新8季的EPS
...
########################################暑假過後，只能估算明年的數字
        ...
        lastYear_Sum = r1+r2+r3+r4+r5+r6+r7+r8+r9+r10+r11+r12 #去年已公布營收 1-12月營收
        ...
        theRest_Predict = lastYear_Sum*(1+RevYoY) #預估今年全年營收
...
####################計算預估EPS
    ...
    Predict_EPS0 = eps1 + eps2 + eps3 + Q4_Predict_EPS #估2020全年EPS 2021/3/31前 尚未公佈
#print(Predict_EPS)
...
#print(New_down_loss)
    ...
    return H1, H2, H3, H4, H5, L1, L2, L3, L4, L5, eps1, eps2, eps3, eps4, eps5, PER_H1, PER_H2, PER_H3, PER_H4, PER_H5, PER_L1, PER_L2, PER_L3, PER_L4, PER_L5, PER_H_average, PER_L_average, PER_H, PER_L, rYoY1N, rYoY2N, rYoY3N, rYoY4N, rYoY5N, rYoY6N, rYoY1, rYoY2, rYoY3, rYoY4, rYoY5, rYoY6, RevYoY, rYoY6Average, r1N, r2N, r3N, r4N, r5N, r6N, r7N, r8N, r9N, r10N, r11N, r12N, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, Rev_Predict, Net1N, Net2N, Net3N, Net4N, Net1, Net2, Net3, Net4, Net4Average, Net_Predict, capital_stock, Predict_EPS, Predict_high_price, Predict_low_price, yahoo_latest_tradePrice, New_up_profit, New_down_loss, risk_reward, pYoY1, pYoY2, pYoY3, pYoY4, pYoY5, pYoY6, pRevYoY, pYoY6Average, pNet1, pNet2, pNet3, pNet4, pNet4Average, H0, thisYear_Sum, theRest_Predict, H6, L6, Predict_EPS0
...
#print(RevYoY)
    ...
    elif (month_id == '6'):  #第一季(Q1)財報：5/15前
        ##########################月份名稱############
        r1N = dfs[2][0][6] #千元 改 億  最新月份名稱
        r2N = dfs[2][0][7]  
        r3N = dfs[2][0][8]
...
###############預估 淨利 EPS  ###2021/3/31公佈2020年報後 必須修改 不用估算 直接抓EPS
    Net_Predict0 = round(Rev_LastYear*Net4Average,6)  #估算2020
...
####################計算預估EPS 2021/2/2修改
    ...
    Predict_EPSq = eps1 + eps2 + eps3 + Q4_Predict_EPS
